...
#define M1120_DETECT_RANGE_HIGH				(-511)
#define M1120_DETECT_RANGE_LOW				(-512)
...
/* static variable */
/* ********************************************************* */
#define ALLOW_THRESHOLD_LOW   -512
static m1120_data_t *p_m1120_data = NULL;
static short m1120_lthd_gain = 0;
static short m1120_hthd_gain = 8;
static int g_is_back = -1;   // 1: back   0: front   -1:invalid
...
static int m1120_set_power(struct device *dev, bool on)
{
	...
	if (regulator_count_voltages(p_data->power_vi2c) > 0) {
		rc = regulator_set_voltage(p_data->power_vi2c, 1800000,
					   1800000);
		if (rc) {
			dev_err(&p_data->client->dev,
...
static void m1120_get_adc_datas(short *adc_max, short *adc_min, int count)
{
    int cnt = 0;
    int max = 0, min = 0;
    ...
    err = m1120_measure(p_m1120_data, &raw);
    max = raw;
    min = raw;
    for (cnt = 0; cnt < count - 1; cnt++)
    {
        ...
        if (!err)
        {
            p_m1120_data->calibrated_data = raw;
        }
...
static int m1120_set_calibration(struct device *dev)
{
    ...
    p_data->thrhigh = max + m1120_hthd_gain;
    p_data->thrlow = max + m1120_lthd_gain;
...
static ssize_t m1120_cali_show(struct device *dev,
                              struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    m1120_data_t *p_data = i2c_get_clientdata(client);
...
static ssize_t m1120_cali_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
    int ret = 0;
    int low_thd = 0, high_thd = 0;
    printk("%s %s", __FUNCTION__, buf);
    if ((ret = sscanf(buf, "%d:%d", &low_thd, &high_thd)) != 2)
    {
        printk("%s  error \n", __FUNCTION__);
        return -1;
    }
...
static DEVICE_ATTR(cali, S_IRUGO | S_IWUSR | S_IWGRP,
                   m1120_cali_show, m1120_cali_store);
