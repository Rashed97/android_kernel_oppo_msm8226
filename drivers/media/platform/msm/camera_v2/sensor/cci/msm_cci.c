	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	if (rc <= 0) {
		...
		for(i = 0; i < cmd_size; i++){
			pr_err("%s: addr 0x%x data 0x%x, delay: %d\n",__func__,i2c_cmd->reg_addr,
...
#ifdef VENDOR_EDIT
	//LiuBin@Camera, 2014/06/11, Add for cci debug
